...
Public Class Form2
    ...
    Dim GameB(16) As Button         '每一個格子(按鈕)
    ...
    Public BlockGameB(16) As Integer   '已被選取的格子
    ...
    Public Function checkLine()
        ...
        For i = 0 To 12 Step 4
            ...
            While j < i + 4                 '水平線
                ...
                j += 1
            End While
            If countOneLine = 4 Then         '如果都被圈走則成為一條線 線+1
                HowmanyLine += 1
            End If
        Next
        For i = 0 To 3
    ...
    Private Sub receiveThreadProc()
        Try
            ...
            Do
                ...
                Select Case tmpStr.Substring(0, 4)
                    Case "RADY"
                        Dim msg As Byte() = Encoding.Default.GetBytes("Strt" & f2.GAMEMODE)
                        networkStream.Write(msg, 0, msg.Length)
                        'clientSocket.Send(msg, 0, msg.Length, SocketFlags.None)
                        Console.WriteLine("sendMode")
                    Case "NumB"
                        f2.setLabel3("Please Choose number")
                        Console.WriteLine("recNumb+" & tmpStr.Substring(4))
                        f2.CanceledBlock("")
                        f2.setGameNumber(tmpStr.Substring(4))
                        f2.setLabel2("目前有" & f2.checkLine() & "條連線")
                        Console.WriteLine("set completely ")
                        Dim msg As Byte() = Encoding.Default.GetBytes("NowL" & f2.Winline)
                        networkStream.Write(msg, 0, msg.Length)
                        'clientSocket.Send(msg, 0, msg.Length, SocketFlags.None)
                        If f2.checkWin(f2.Winline) Then
                            f2.setLabel2("You Win")
                        End If
                    Case "NowL"
                        If f2.checkWin(tmpStr.Substring(4)) Then
                            If f2.GameOver = False Then
                                f2.setLabel2("You Lose")
                            End If
                        End If
                    Case "GGGG"
                        f2.GGSMIIDA()
